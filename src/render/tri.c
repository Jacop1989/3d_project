#include "tri.h"
#include <math.h>

static double edge_function(const Vec3 *a, const Vec3 *b, double x, double y) {
    return (x - a->x) * (b->y - a->y) - (y - a->y) * (b->x - a->x);
}

void tri_fill(Framebuffer *fb, ZBuffer *zb, Vec3 v0, Vec3 v1, Vec3 v2, uint32_t color) {
    if (!fb || !zb) return;
    float minx = floorf(fminf(fminf(v0.x, v1.x), v2.x));
    float maxx = ceilf(fmaxf(fmaxf(v0.x, v1.x), v2.x));
    float miny = floorf(fminf(fminf(v0.y, v1.y), v2.y));
    float maxy = ceilf(fmaxf(fmaxf(v0.y, v1.y), v2.y));

    if (minx < 0) minx = 0;
    if (miny < 0) miny = 0;
    if (maxx > (float)(fb->width - 1)) maxx = (float)(fb->width - 1);
    if (maxy > (float)(fb->height - 1)) maxy = (float)(fb->height - 1);

    double area = edge_function(&v0, &v1, v2.x, v2.y);
    if (fabs(area) < 1e-12) return;

    for (int y = (int)miny; y <= (int)maxy; ++y) {
        for (int x = (int)minx; x <= (int)maxx; ++x) {
            double px = x + 0.5;
            double py = y + 0.5;
            double w0 = edge_function(&v1, &v2, px, py);
            double w1 = edge_function(&v2, &v0, px, py);
            double w2 = edge_function(&v0, &v1, px, py);
            if ((w0 >= 0 && w1 >= 0 && w2 >= 0) || (w0 <= 0 && w1 <= 0 && w2 <= 0)) {
                w0 /= area; w1 /= area; w2 /= area;
                float z = (float)(w0 * v0.z + w1 * v1.z + w2 * v2.z);
                if (zbuf_test_and_write(zb, (size_t)x, (size_t)y, z)) {
                    framebuffer_put_pixel(fb, (size_t)x, (size_t)y, color);
                }
            }
        }
    }
}
